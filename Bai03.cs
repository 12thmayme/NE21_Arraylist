...
public class Program
{
    static void Main(string[] args)
    {
        ...
        var itemsInArrayList = arrayList.OfType<Item>().Where(i => i.Id == 10).ToList();
        if (itemsInArrayList.Count > 0)
        {
            var itemInArrayList = itemsInArrayList.First();
            Console.WriteLine($"Tìm thấy trong ArrayList: Id: {itemInArrayList.Id}, Name: {itemInArrayList.Name}, Position: {arrayList.IndexOf(itemInArrayList)}");
        }

        var itemsInList = itemList.Where(i => i.Id == 10).ToList();
        if (itemsInList.Count > 0)
        {
            var itemInList = itemsInList.First();
            Console.WriteLine($"Tìm thấy trong List<Item>: Id: {itemInList.Id}, Name: {itemInList.Name}, Position: {itemList.IndexOf(itemInList)}");
        }
    }
}
